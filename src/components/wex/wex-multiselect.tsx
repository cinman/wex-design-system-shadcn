          ...
          return (
            <div className="flex flex-wrap gap-1 flex-1 min-w-0 overflow-hidden">
              {selectedOptions.map((option) => (
                <span
                  key={option.value}
                  className="inline-flex items-center gap-1 bg-primary text-primary-foreground text-xs px-2 py-0.5 rounded-full shrink-0"
                >
                  <span className="truncate max-w-[80px]">{option.label}</span>
                  <span
                    role="button"
                    tabIndex={disabled ? -1 : 0}
                    className="inline-flex items-center justify-center rounded-full hover:bg-primary-foreground/20 min-h-6 min-w-6 -mr-1 cursor-pointer"
